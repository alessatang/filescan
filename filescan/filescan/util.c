...
#define getc_unlocked(x) getwc(x)
#endif
...
#define CHECK_AND_RETURN(ptr)             \
    if (ptr == NULL) {                    \
        die(L"Memory allocation failed."); \
    }                                     \
    return ptr;
...
wchar_t *ag_strndup(const wchar_t *s, size_t size) {
    ...
    CHECK_AND_RETURN(str)
#else
	str = (wchar_t *)ag_malloc(size + 1);
	wcscpy_s(str, size + 1, s);
    return str;
#endif
}
...
void generate_alpha_skip(const wchar_t *find, size_t f_len, size_t skip_lookup[], const int case_sensitive) {
    ...
    for (i = 0; i < f_len; i++) {
        if (case_sensitive) {
			skip_lookup[(wchar_t)find[i]] = f_len - i;
        } else {
			skip_lookup[(wchar_t)_tolower(find[i])] = f_len - i;
			skip_lookup[(wchar_t)_toupper(find[i])] = f_len - i;
        }
    }
}
...
const wchar_t *boyer_moore_strnstr(const wchar_t *s, const wchar_t *find, const size_t s_len, const size_t f_len,
    ...
    while (pos < s_len) {
		...
		pos += ag_max(alpha_skip_lookup[( wchar_t)s[pos]], find_skip_lookup[i]);
    }
...
size_t invert_matches(const wchar_t *buf, const size_t buf_len, match_t matches[], size_t matches_len) {
    ...
    log_debug(L"Inverting %u matches.", matches_len);
    ...
    for (i = 0; i < matches_len; i++) {
        log_debug(L"Inverted match %i start %i end %i.", i, matches[i].start, matches[i].end);
    }
...
void compile_study(pcre16 **re, pcre16_extra **re_extra, wchar_t *q, const int pcre_opts, const int study_opts) {
	...
	*re = pcre16_compile((PCRE_SPTR16) q, pcre_opts, &pcre_err, &pcre_err_offset, NULL);
    if (*re == NULL) {
        die(L"Bad regex! pcre_compile() failed at position %i: %s\nIf you meant to search for a literal string, run ag with -Q",
            pcre_err_offset,
            pcre_err);
    }
	*re_extra = pcre16_study(*re, study_opts, &pcre_err);
    if (*re_extra == NULL) {
        log_debug(L"pcre_study returned nothing useful. Error: %s", pcre_err);
    }
}
...
int is_binary(const wchar_t* buf, const size_t buf_len) {
    ...
    const wchar_t *buf_c = (const wchar_t *)buf;
    size_t i;
    ...
    if (buf_len >= 4 && wcsncmp(buf, L"%PDF-", 5) == 0) {
        /* PDF. This is binary. */
        return 1;
    }
...
int is_regex(const wchar_t *query) {
    ...
    return (wcspbrk(query, regex_chars) != NULL);
}

int is_fnmatch(const wchar_t *filename) {
    wchar_t fnmatch_chars[] = {
        '!',
        '*',
        '?',
        '[',
    ...
    return (wcspbrk(filename, fnmatch_chars) != NULL);
}
...
int is_wordchar(wchar_t ch) {
	return wordchar_table[(wchar_t)ch];
}
...
int is_directory(const wchar_t *path, const struct dirent *d) {
	...
	wchar_t *full_path;
    struct _stat s;
    ag_asprintf(&full_path, L"%s/%s", path, d->d_name);
    if (_wstat(full_path, &s) != 0) {
        free(full_path);
        return FALSE;
    }
    free(full_path);
...
int Unicode2UTF8(const wchar_t *pSource, char** pDest, int *destSize)
{
	...
	int utf8Len = WideCharToMultiByte(CP_UTF8, 0, pSource, wcslen(pSource) + 1, NULL, 0, NULL, NULL);
	if (utf8Len == 0)
	{
		log_err(L"Unicode2UTF8: failed to convert to utf8.");
		return 0;
	}
	char *pUTF8 = (char *)malloc(utf8Len);
	memset(pUTF8, 0, utf8Len);
	WideCharToMultiByte(CP_UTF8, 0, pSource, wcslen(pSource) + 1, pUTF8, utf8Len, NULL, NULL);
...
{
	int unicodeLen = MultiByteToWideChar(CP_UTF8, 0, pSource, strlen(pSource) + 1, NULL, 0);
	if (unicodeLen == 0)
	{
		log_err(L"UTF82Unicode: failed to convert to utf8.");
		return 0;
	}
	wchar_t *pUnicode = (wchar_t *)malloc(unicodeLen * sizeof(wchar_t));
	memset(pUnicode, 0, sizeof(wchar_t) * unicodeLen);

	MultiByteToWideChar(CP_UTF8, 0, pSource, strlen(pSource) + 1, pUnicode, unicodeLen);
	*pDest = pUnicode;
	*destSize = unicodeLen;
	return 1;
}
...
int getTempFile(wchar_t *tmp_file)
{
	if (tmp_file == NULL)
	{
		log_err(L"getTempFile:tmp_file is nULL");
		return 0;
	}
	wchar_t lpTmpFilePath[MAX_PATH];
	DWORD dwRet = 0;
	UINT uRet = 0;
	HANDLE hTempFile = INVALID_HANDLE_VALUE;

	dwRet = GetTempPath(MAX_PATH, lpTmpFilePath);
	if (dwRet > MAX_PATH || (dwRet == 0))
	{
		log_err(L"convert_to_text: GetTempPath failed.\n");
		return 0;

	}
	uRet = GetTempFileName(lpTmpFilePath, NULL, 0, (LPTSTR)tmp_file);
	if (uRet == 0)
	{
		log_err(L"GetTempFileName failed.\n");
		return 0;
	}
...
wchar_t *fgetln(FILE *fp, size_t *lenp) {
    ...
    flockfile(fp);
    while ((wc = getwc(fp)) != WEOF) {
        if (!buf || len >= used) {
            size_t nsize;
			wchar_t *newbuf;
            nsize = used + BUFSIZ;
			if (!(newbuf = (wchar_t*)realloc(buf, nsize*sizeof(wchar_t)))) {
                funlockfile(fp);
                if (buf)
                    free(buf);
                return NULL;
...
ssize_t getline(wchar_t **lineptr, size_t *n, FILE *stream) {
    ...
    if (len >= *n) {
		...
		if (!(newlnptr = (wchar_t*)realloc(*lineptr, len * 2 *sizeof(wchar_t)))) {
            return -1;
        }
        *lineptr = newlnptr;
        *n = len * 2;
    ...
    memcpy(*lineptr, srcln, len*sizeof(wchar_t));
...
wchar_t *realpath(const wchar_t *path, wchar_t *resolved_path) {
    ...
    return _wfullpath(resolved_path, tmp, _MAX_PATH);
}
#endif
...
#endif
    rv = _vsnwprintf(NULL, 0, fmt, args);
    va_end(args);
    if (rv < 0) {
        return rv;
    }
    *ret = (wchar_t*)malloc(++rv * sizeof(wchar_t)); /* vsnprintf doesn't count \0 */
    if (*ret == NULL) {
        return -1;
    }
	rv = _vsnwprintf(*ret, rv, fmt, args2);
    va_end(args2);
    if (rv < 0) {
        free(*ret);
    }
