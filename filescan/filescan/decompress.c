...
static void *decompress_lzw(const void *buf, const int buf_len,
                            const wchar_t *dir_full_path, int *new_buf_len) {
    (void)buf;
    (void)buf_len;
    log_err(L"LZW (UNIX compress) files not yet supported: %s", dir_full_path);
    *new_buf_len = 0;
    return NULL;
}
...
static void *decompress_zip(const void *buf, const int buf_len,
    ...
    (void)buf;
    (void)buf_len;
    log_err(L"Zip files not yet supported: %s", dir_full_path);
    *new_buf_len = 0;
    return NULL;
}
...
void *decompress(const ag_compression_type zip_type, const void *buf, const int buf_len,
    ...
    switch (zip_type) {
        ...
        case AG_NO_COMPRESSION:
            log_err(L"File %s is not compressed", dir_full_path);
            break;
        default:
            log_err(L"Unsupported compression type: %d", zip_type);
    }
...
ag_compression_type is_zipped(const void *buf, const int buf_len) {
    ...
    if (buf_len >= 2) {
        if (buf_c[0] == 0x1F) {
            if (buf_c[1] == 0x8B) {
#ifdef HAVE_ZLIB_H
                log_debug(L"Found gzip-based stream");
                return AG_GZIP;
#endif
            } else if (buf_c[1] == 0x9B) {
                log_debug(L"Found compress-based stream");
                return AG_COMPRESS;
            }
        }
    }
    ...
    if (buf_len >= 4) {
        if (buf_c[0] == 0x50 && buf_c[1] == 0x4B && buf_c[2] == 0x03 && buf_c[3] == 0x04) {
            log_debug(L"Found zip-based stream");
            return AG_ZIP;
        }
    }
    ...
    if (buf_len >= 6) {
        if (memcmp(XZ_HEADER_MAGIC, buf_c, 6) == 0) {
            log_debug(L"Found xz based stream");
            return AG_XZ;
        }
    }
    ...
    if (buf_len >= 3) {
        if (memcmp(LZMA_HEADER_SOMETIMES, buf_c, 3) == 0) {
            log_debug(L"Found lzma-based stream");
            return AG_XZ;
        }
    }
#endif
