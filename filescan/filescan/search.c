...
void search_buf(wchar_t *buf, size_t buf_len,
	const wchar_t *dir_full_path, wchar_t *tmp_file_path) {
	...
	if (opts.search_stream) {
		...
		//     if (binary) {
		//         log_debug(L"File %s is binary. Skipping...", dir_full_path);
	...
	if (!opts.literal && opts.query_len == 1 && opts.query[0] == '.') {
		...
		while (buf_offset < buf_len) {
			...
			if (opts.word_regexp) {
				const wchar_t *start = match_ptr;
				const wchar_t *end = match_ptr + opts.query_len;
			...
			if (matches_len + matches_spare >= matches_size) {
				...
				log_debug(L"Too many matches in %s. Reallocating matches to %zu.", dir_full_path, matches_size);
			...
			log_debug(L"Match found. File %s, offset %lu bytes.", dir_full_path, matches[matches_len].start);
			matches_len++;
			match_ptr += opts.query_len;

			if (opts.max_matches_per_file > 0 && matches_len >= opts.max_matches_per_file) {
				log_err(L"Too many matches in %s. Skipping the rest of this file.", dir_full_path);
				break;
			}
		}
	}
		...
		int offset_vector[3];
		while (buf_offset < buf_len &&
			(pcre16_exec(opts.re, opts.re_extra, (PCRE_SPTR16)buf, buf_len/sizeof(wchar_t), buf_offset, 0, offset_vector, 3)) >= 0) {
			log_debug(L"Regex match found. File %s, offset %i bytes.", dir_full_path, offset_vector[0]);
			buf_offset = offset_vector[1];
			if (offset_vector[0] == offset_vector[1]) {
				++buf_offset;
				log_debug(L"Regex match is of length zero. Advancing offset one byte.");
			}
	...
	if (matches_len > 0) {
		...
		log_debug(L"No match in %s", dir_full_path);
	}
...
void search_stream(FILE *stream, const wchar_t *path) {
	wchar_t *line = NULL;
	ssize_t line_len = 0;
	size_t line_cap = 0;
	size_t i;
...
FILETYPES get_file_type(const wchar_t *file_full_path)
{
	wchar_t *file = (wchar_t*)wcsrchr(file_full_path, '\\') + 1;
	wchar_t *ext = (wchar_t*)wcsrchr(file, '.') + 1;
	if (ext != NULL)
	{
		_wcslwr(ext);
		if (wcscmp(ext, L"pdf") == 0 || wcscmp(ext, L"docx") == 0 || wcscmp(ext, L"doc") == 0 || wcscmp(ext, L"rtf") == 0 ||
			wcscmp(ext, L"pptx") == 0 || wcscmp(ext, L"ppt") == 0 || wcscmp(ext, L"xlsx") == 0 || wcscmp(ext, L"xls") == 0 ||
			wcscmp(ext, L"xml") == 0 || wcscmp(ext, L"html") == 0 || wcscmp(ext, L"htm") == 0 || wcscmp(ext, L"odt") == 0 ||
			wcscmp(ext, L"ods") == 0 || wcscmp(ext, L"odp") == 0 || wcscmp(ext, L"odg") == 0 || wcscmp(ext, L"ppsx") == 0 ||
			wcscmp(ext, L"xlsb") == 0 || wcscmp(ext, L"pps") == 0 || wcscmp(ext, L"eml") == 0 || wcscmp(ext, L"pages") == 0 ||
			wcscmp(ext, L"key")==0 || wcscmp(ext, L"numbers"))
		{
			return DOCUMENTS;
		}
		else if (wcscmp(ext, L"txt") == 0 || wcscmp(ext, L"text") == 0)
...
void search_file(const wchar_t *file_full_path) {
	int fd;
	off_t f_len = 0;
	wchar_t *buf = NULL;
	...
	int rv = 0;
	FILE *pipe = NULL;
	wchar_t* tmp_file_path = NULL;

	tmp_file_path = (wchar_t *)ag_malloc(MAX_PATH * sizeof(wchar_t));
	memset(tmp_file_path, 0, sizeof(wchar_t)*MAX_PATH);
	FILETYPES type = get_file_type(file_full_path);
	if (type == DOCUMENTS)
	{
		if (!convert_to_text(file_full_path, tmp_file_path))
		{
			log_err(L"Failed to convert to text.");
			goto cleanup;
		}

		fd = _wopen(tmp_file_path, _O_RDONLY);
	}
	else if (type ==TEXT)
	{
		fd = _wopen(file_full_path, _O_RDONLY);
	}
	else
	{
		log_debug(L"Do not search for not documents");
		goto cleanup;
	}
	...
	if (fd < 0) {
		...
		log_err(L"Skipping %s: Error opening file: %s", file_full_path, strerror(errno));
		goto cleanup;
	}

	rv = _fstat(fd, &statbuf);
	if (rv != 0) {
		log_err(L"Skipping %s: Error fstat()ing file.", file_full_path);
		goto cleanup;
	}

	if (opts.stdout_inode != 0 && opts.stdout_inode == statbuf.st_ino) {
		log_debug(L"Skipping %s: stdout is redirected to it", file_full_path);
		goto cleanup;
	}

	if ((statbuf.st_mode & S_IFMT) == 0) {
		log_err(L"Skipping %s: Mode %u is not a file.", file_full_path, statbuf.st_mode);
		goto cleanup;
	}

	if (statbuf.st_mode & S_IFIFO) {
		log_debug(L"%s is a named pipe. stream searching", file_full_path);
		pipe = fdopen(fd, "r");
	...
	if (f_len == 0) {
		log_debug(L"Skipping %s: file is empty.", file_full_path);
		goto cleanup;
	}

	if (!opts.literal && f_len > INT_MAX) {
		log_err(L"Skipping %s: pcre16_exec() can't handle files larger than %i bytes.", file_full_path, INT_MAX);
		goto cleanup;
	}
...
#ifdef _WIN32
	{
		...
		buf = (wchar_t *)MapViewOfFile(hmmap, FILE_SHARE_READ, 0, 0, f_len);
		if (hmmap != NULL)
			CloseHandle(hmmap);
	}
	if (buf == NULL) {
		...
		log_err(L"File %s failed to load: %s.", file_full_path, buf);
		LocalFree((void *)buf);
		goto cleanup;
	}
#else
	...
	if (opts.search_zip_files) {
		...
		if (zip_type != AG_NO_COMPRESSION) {
			...
			wchar_t *_buf = (wchar_t*)decompress(zip_type, buf, f_len, file_full_path, &_buf_len);
			if (_buf == NULL || _buf_len == 0) {
				log_err(L"Cannot decompress zipped file %s", file_full_path);
				goto cleanup;
			}
...
void *search_file_worker(void *i) {
	...
	log_debug(L"Worker %i started", worker_id);
	while (TRUE) {
		pthread_mutex_lock(&work_queue_mtx);
		while (work_queue == NULL) {
			if (done_adding_files) {
				pthread_mutex_unlock(&work_queue_mtx);
				log_debug(L"Worker %i finished.", worker_id);
				pthread_exit(NULL);
			}
...
static int check_symloop_enter(const wchar_t *path, dirkey_t *outkey) {
#ifdef _WIN32
	return SYMLOOP_OK;
#else
	struct stat buf;
 ...
 */
void search_dir(ignores *ig, const wchar_t *base_path, const wchar_t *path, const int depth,
	dev_t original_dev) {
	...
	wchar_t *dir_full_path = NULL;
	const wchar_t *ignore_file = NULL;
	int i;
	...
	if (symres == SYMLOOP_LOOP) {
		log_err(L"Recursive directory loop: %s", path);
		return;
	}
	...
	for (i = 0; opts.skip_vcs_ignores ? (i == 0) : (ignore_pattern_files[i] != NULL); i++) {
		...
		ag_asprintf(&dir_full_path, L"%s/%s", path, ignore_file);
		if (wcscmp(SVN_DIR, ignore_file) == 0) {
	...
	scandir_baton.base_path_len = base_path ? wcslen(base_path) : 0;
	results = ag_scandir(path, &dir_list, &filename_filter, &scandir_baton);
	if (results == 0) {
		log_debug(L"No results found in directory %s", path);
		...
		if (errno == ENOTDIR) {
			...
			log_err(L"Error opening directory %s: %s", path, strerror(errno));
		}
		goto search_dir_cleanup;
	}
	...
	for (i = 0; i < results; i++) {
		queue_item = NULL;
		dir = dir_list[i];
		ag_asprintf(&dir_full_path, L"%s\\%s", path, dir->d_name);
#ifndef _WIN32
		if (opts.one_dev) {
			struct stat s;
		...
		if (!is_directory(path, dir)) {
			if (opts.file_search_regex) {
				rc = pcre16_exec(opts.file_search_regex, NULL, (PCRE_SPTR16)dir_full_path, wcslen(dir_full_path),
					0, 0, offset_vector, 3);
				if (rc < 0) { /* no match */
					log_debug(L"Skipping %s due to file_search_regex.", dir_full_path);
					goto cleanup;
				}
				else if (opts.match_files) {
					log_debug(L"match_files: file_search_regex matched for %s.", dir_full_path);
					pthread_mutex_lock(&print_mtx);
					print_path(dir_full_path, opts.path_sep);
					pthread_mutex_unlock(&print_mtx);
					opts.match_found = 1;
			...
			if (depth < opts.max_search_depth || opts.max_search_depth == -1) {
				...
				if (opts.max_search_depth == DEFAULT_MAX_SEARCH_DEPTH) {
					 ...
					 * this is a warning...
					 */
					log_warn(L"Skipping %s. Use the --depth option to search deeper.", dir_full_path);
				}
				else {
					/* ... if they did, let's settle for debug. */
					log_debug(L"Skipping %s. Use the --depth option to search deeper.", dir_full_path);
				}
			}
		}
